...
// define signal leds turnout 1 pins 0 and 1 rx & tx ADDRESS 6001 USED FOR BIT STATE OF TURNOUT
#define throughApproachGreenLed 4                 //jmri 6002
#define throughApproachRedLed 5                   //jmri 6003
#define divergingApproachGreenLed 6               //jmri 6004
#define divergingApproachRedLed 7                 //jmri 6005
#define throughGreenLed 8                         //jmri 6006
#define throughRedLed 9                           //jmri 6007

// Spare pins at the moment 3 10 to 13 And A5 to A7 

//define infrared detection T3 as inputs
#define Sensor1 A0                                //jmri 6002
#define Sensor2 A1                                //jmri 6003
#define Sensor3 A2                                //jmri 6004
#define Sensor4 A3                                //jmri 6005
#define Sensor4 A4                                //jmri 6006
...
void loop() {
  cmri.process();
  turnout1 = (cmri.get_bit(0));                                             //turnout 6001 jmri address

  if (turnout1 == 1 && t1state == 0) {
    turnout1Target = turnout1ThrownPosition;   
    t1state = 1;
    cmri.set_bit(0, HIGH);                                                  //6001 jmri address output
  }
  if (turnout1 == 0 && t1state == 1) {
    turnout1Target = turnout1ClosedPosition;    
    t1state = 0;
    cmri.set_bit(0, LOW);                                                   //6001
  }

  // read the cmri bit and switch on led ! used as i am using + commom leds tmc signals
  digitalWrite (throughApproachGreenLed, !cmri.get_bit(1));                 //jmri SH1 6002 
  digitalWrite (throughApproachRedLed, !cmri.get_bit(2));                   //jmri SH1 6003
  digitalWrite (divergingApproachGreenLed, !cmri.get_bit(3));               //jmri SH2 6004
  digitalWrite (divergingApproachRedLed, !cmri.get_bit(4));                 //jmri SH2 6005
  digitalWrite (throughGreenLed, !cmri.get_bit(5));                         //jmri SH3 6006
  digitalWrite (throughRedLed, !cmri.get_bit(6));                           //jmri SH3 6007

  // get senors status and send to jmri
  cmri.set_bit(1, !digitalRead(A0));                                        //jmri 6002
  cmri.set_bit(2, !digitalRead(A1));                                        //jmri 6003
  cmri.set_bit(3, !digitalRead(A2));                                        //jmri 6004
  cmri.set_bit(4, !digitalRead(A3));                                        //jmri 6005
  cmri.set_bit(5, !digitalRead(A4));                                        //jmri 6006
